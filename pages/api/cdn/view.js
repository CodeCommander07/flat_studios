...
export default async function handler(req, res) {
  ...
  const { fileId, userId, download, embed } = req.query;
  if (!fileId || !userId) {
  ...
  try {
    ...
    const displayName = (fileDoc.filename || 'file').replace(
    ...
    // === Handle Google Docs viewer (optional embed=true) ===
    if (embed && (officeDocs.includes(ext) || pdfExts.includes(ext))) {
    ...
    // === Serve inline image / PDF preview ===
    ...
    // === Office docs inline (so React blob preview works) ===
    if (officeDocs.includes(ext)) {
      res.setHeader('Content-Type', fileDoc.contentType || 'application/vnd.openxmlformats-officedocument.wordprocessingml.document');
      res.setHeader('Content-Disposition', `inline; filename="${displayName}"`);
      return res.status(200).send(fileDoc.data);
    }

    // === Default fallback: download mode ===
    if (download === 'true') {
