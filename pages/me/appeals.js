'use client';
import { useEffect, useState } from 'react';
import { motion } from 'framer-motion';
import { Gavel, Loader2 } from 'lucide-react';

export default function AppealsHistory() {
  const [appeals, setAppeals] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function load() {
      try {
        const user = localStorage.getItem('User');
        const parsedUser = JSON.parse(user);
        const res = await fetch(`/api/appeals/user?email=${parsedUser.email}`);
        const data = await res.json();
        setAppeals(data.appeals || []);
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
      }
    }
    load();
  }, []);

  return (
    <div className="text-white p-6">
      <motion.div
        initial={{ opacity: 0, y: 30 }}
        animate={{ opacity: 1, y: 0 }}
        className="max-w-4xl mx-auto bg-white/10 backdrop-blur-xl rounded-2xl p-8 border border-white/10 shadow-xl"
      >
        <h1 className="text-3xl font-bold mb-6">Your Appeals History</h1>

        {loading ? (
          <div className="flex justify-center py-10">
            <Loader2 className="animate-spin w-8 h-8 text-gray-400" />
          </div>
        ) : appeals.length === 0 ? (
          <p className="text-gray-400 text-center py-10">
            You haven’t submitted any appeals yet.
          </p>
        ) : (
          <div className="space-y-4">
            {appeals.map((appeal) => (
              <motion.div
                key={appeal._id}
                whileHover={{ scale: 1.02 }}
                className="flex justify-between items-center bg-white/5 p-4 rounded-xl border border-white/10"
              >
                <div className="flex items-center gap-3">
                  <Gavel className="text-purple-400 w-5 h-5" />
                  <div>
                    <p className="font-semibold">{appeal.reason || 'Appeal Submitted'}</p>
                    <p className="text-sm text-gray-400">
                      Submitted: {new Date(appeal.createdAt).toLocaleDateString()}
                    </p>
                  </div>
                </div>
                <span
                  className={`px-3 py-1 rounded-md text-sm font-semibold ${
                    appeal.status === 'accepted'
                      ? 'bg-green-600/40 text-green-300'
                      : appeal.status === 'denied'
                      ? 'bg-red-600/40 text-red-300'
                      : 'bg-yellow-600/40 text-yellow-300'
                  }`}
                >
                  {appeal.status?.toUpperCase() || 'PENDING'}
                </span>
              </motion.div>
            ))}
          </div>
        )}
      </motion.div>
    </div>
  );
}
