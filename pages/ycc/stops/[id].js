...
export default function StopDetailPage() {
  ...
  const fetchStop = async () => {
    ...
    try {
      ...
      // ✅ Normalize booleans
      if (typeof stopData.closed === 'string') {
        stopData.closed = stopData.closed.toLowerCase() === 'true';
      }
      if (stopData.diversion && typeof stopData.diversion.active === 'string') {
        stopData.diversion.active = stopData.diversion.active.toLowerCase() === 'true';
      }
  ...
  if (loading) return <p className="text-white text-center mt-12">Loading stop details...</p>;
  if (error) return <p className="text-red-500 text-center mt-12">{error}</p>;
  if (!stop) return <p className="text-white text-center mt-12">Stop not found.</p>;
      ...
      <div className="max-w-4xl w-full bg-white/10 border border-white/20 backdrop-blur-md p-6 rounded-2xl shadow-xl">
        ...
        {routes.length > 0 ? (() => {
          ...
          const servingRoutes = routes.filter((r) => {
            ...
            return allStops.includes(stop.stopId); // ✅ match by stopId
          });

          if (servingRoutes.length === 0) {
            return <p className="mt-4 text-gray-400">No routes currently assigned.</p>;
          }
