  ...
  return (
    <main className="max-w-10xl mx-auto p-6 text-white grid md:grid-cols-2 gap-6">
      ...
      <div className="flex flex-col gap-6">
        <div className="bg-[#283335] p-5 rounded-xl flex flex-col md:flex-row items-center md:items-start gap-6 border" style={{ borderColor: operator.operatorColour }}>
          {operator.logo && (
            <Image
              src={operator.logo}
        ...
        {robloxGroup && (
          <div className="bg-[#283335] rounded-xl p-5 border" style={{ borderColor: operator.operatorColour }}>
      ...
      {/* RIGHT — Routes */}
      <div className="bg-[#283335] rounded-xl p-5 relative border" style={{ borderColor: operator.operatorColour }}>
        <h2 className="text-xl font-semibold mb-3">
          Routes operated by {operator.operatorName}
        </h2>

        <h2 className='text-xl text-orange-400'>Unfortunatly Route logic is temporarily disabled.</h2>
        <h3 className='text-lg'>Estimated Completion Date: <span className='underline'>Wednesday 3rd December 2025</span></h3>

{/* 🔥 ROUTES MAP BLOCK (DISABLED) 🔥
{routesLoading ? (
    ...
    {routes.map((r) => {
      ...
      const operatorText = Array.isArray(r.operator)
        ? r.operator
            .map(
              (id) =>
                operators.find((o) => o._id === id)?.operatorName ||
                "Unknown"
            )
            .join(", ")
        : operators.find((o) => o._id === r.operator)?.operatorName ||
          "Unknown";
