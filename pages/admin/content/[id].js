        ...
        <main className="p-8 max-w-6xl mx-auto text-white space-y-8">
            ...
            <Section icon={<Eye className="text-indigo-400" />} title="Visibility & Status">
                ...
                {form.status === 'scheduled' && (
                    <div className="mt-2">
                        <DatePicker
                            selected={new Date(form.scheduledFor || Date.now())}
                            onChange={(date) => onChange('scheduledFor', date.toISOString())}
                            showTimeSelect
                            timeFormat="HH:mm"
                            timeIntervals={15}
                            dateFormat="MMMM d, yyyy h:mm aa"
                            className="p-2 rounded-md bg-black/30 border border-white/10 text-white"
                        />
