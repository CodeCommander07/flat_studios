  ...
  return (
    <AuthWrapper requiredRole="admin">
      <main className="max-w-6xl mx-auto mt-10 p-8 bg-white/10 border border-white/20 rounded-2xl text-white backdrop-blur-lg shadow-lg">
        <h1 className="text-2xl font-bold mb-6">Route Requests</h1>
        ...
        {requests.length === 0 ? (
          ...
          <div className="divide-y divide-white/10">
            {requests.map((req) => (
              <Link
                key={req._id}
                href={`/admin/ycc/routes/${req._id}`} // keep your existing target route
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-400">
                      ...
                      {req.meta?.submissionType ? (
                        <span className="ml-2 text-gray-500">• {req.meta.submissionType}</span>
                      ) : null}
                    </p>
                  </div>
